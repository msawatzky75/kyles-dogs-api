...
module V1
  class ProductsController < ApiController
    ...
    def index
      ...
      @products = @products.where(product_status_code: ProductStatusCode.where('name ILIKE ?', params[:product_status_code_name]).first) if params[:product_status_code_name].present? && !params[:product_status_code_name].casecmp('all').zero?
